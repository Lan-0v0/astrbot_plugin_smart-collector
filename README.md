...
- Pixiv 条目支持多 Tag 搜索、非 R18/R18、非 AI/AI、原图/大图/中图组合筛选，以及二维码 OAuth 登录。
...
- 网站条目可开启“视频内容仅发送URL”：保留视频筛选和画质排序，只发送实体视频 URL 文本，插件与机器人协议端均不主动把它作为视频上传。
...
发送，并按同一用户 1 秒限速。
Pixiv 自定义条目的“参数设定”可多选非 R18、R18、非 AI、AI及原图/大图/中图画质；
默认勾选非 R18、R18、非 AI和原图。勾选多个画质时按原图、大图、中图顺序使用首个可用规格。
Pixiv 的“图片转 PDF”是独立三选项：不启用、所有图片转 PDF、R18 图片转 PDF，默认仅转换 R18；
网站和 API 条目的“图片转 PDF”仍保持原有开关。
...
网站条目的“视频内容仅发送URL”默认关闭；开启后以纯文本发送可直接访问的实体视频 URL，不发送
HLS/DASH 清单，也不会下载或压缩 URL 模式的视频；
...
当前版本：`v0.4.1`。变更记录见 [CHANGELOG.md](CHANGELOG.md)。
